...
export const getFileLogo = (filename, mimeType) => {
  ...
  const codeExtensions = [
    ...
    'go', 'rs', 'php', 'html', 'htm', 'css', 'scss', 'sass', 'less', 'sh', 'bash',
    'zsh', 'sql', 'json', 'xml', 'yaml', 'yml', 'rb', 'swift', 'kt', 'scala', 'm',
    'mm', 'r', 'pl', 'lua', 'vim', 'emacs', 'clj', 'cljs', 'ex', 'exs', 'erl',
    'hrl', 'fs', 'fsx', 'fsi', 'ml', 'mli', 'pas', 'delphi', 'asm', 'groovy',
    'gradle', 'maven', 'make', 'cmake', 'dockerfile'
  ];
  ...
  if (mime.includes('text/') || mime.includes('application/x-') || mime === 'application/octet-stream') {
    // Additional check for known code MIME types
    if (mime.includes('json') || mime.includes('xml') || mime.includes('sql') || mime.includes('javascript')) {
      return '/file-logos/code.svg';
    }
  }
...
export const getFileCategory = (filename, mimeType) => {
  ...
  if (extension.match(/^(jpg|jpeg|png|gif|webp|bmp|svg|ico|tiff|raw|psd|ai|eps)$/)) {
    return 'image';
  }
  if (extension.match(/^(mp3|wav|aac|flac|m4a|ogg|wma|aiff)$/) || mime.startsWith('audio/')) {
    return 'audio';
  }
  if (extension.match(/^(mp4|webm|avi|mkv|mov|flv|wmv|3gp|m3u8|mpeg|mid)$/) || mime.startsWith('video/')) {
    return 'video';
  }
  if (extension.match(/^(zip|rar|7z|tar|gz|bz2|xz|iso)$/) || mime.includes('archive')) {
    return 'archive';
  }
  if (extension.match(/^(doc|docx|odt|txt|rtf|pdf)$/) || mime.includes('document') || mime.includes('word')) {
    return 'document';
  }
  ...
  if (extension.match(/^(js|jsx|ts|tsx|py|java|c|cpp|go|rs|php|html|css|sh|sql|json|xml|yaml|yml)$/)) {
    return 'code';
  }
