...
export const getFileIcon = (filename, mimeType, size = 24) => {
  ...
  // Image files
  if (extension.match(/^(jpg|jpeg|png|gif|webp|bmp|svg|ico)$/) || mime.startsWith('image/')) {
    return (
      <Image 
        size={size} 
  ...
  // Word/Document files
  if (extension.match(/^(doc|docx|odt)$/) || mime.includes('word') || mime.includes('document')) {
    return (
      <FileText 
        size={size} 
  ...
  if (extension.match(/^(xls|xlsx|csv|ods)$/) || mime.includes('spreadsheet') || mime.includes('sheet')) {
    return (
      <FileSpreadsheet 
        size={size} 
  ...
  if (extension.match(/^(js|jsx|ts|tsx)$/) || mime === 'text/javascript' || mime === 'application/javascript') {
    return (
      <FileCode 
        size={size} 
  ...
  // Code files - C/C++
  if (extension.match(/^(c|cpp|cc|cxx|h|hpp)$/) || mime === 'text/x-c' || mime === 'text/x-cpp') {
    return (
      <FileCode 
        size={size} 
  ...
  // Code files - HTML/CSS
  if (extension.match(/^(html|htm|css|scss|sass|less)$/) || mime.includes('html') || mime.includes('css')) {
    return (
      <FileCode 
        size={size} 
  ...
  // Code files - XML/YAML
  if (extension.match(/^(xml|yaml|yml)$/) || mime.includes('xml') || mime.includes('yaml')) {
    return (
      <FileCode 
        size={size} 
  ...
  // Archive files
  if (extension.match(/^(zip|rar|7z|tar|gz|bz2|xz|iso)$/) || mime.includes('archive') || mime.includes('compressed')) {
    return (
      <FileArchive 
        size={size} 
  ...
  // Audio files
  if (extension.match(/^(mp3|wav|aac|flac|m4a|ogg|wma|aiff)$/) || mime.startsWith('audio/')) {
    return (
      <Music 
        size={size} 
  ...
  // Video files
  if (extension.match(/^(mp4|webm|avi|mkv|mov|flv|wmv|3gp|m3u8)$/) || mime.startsWith('video/')) {
    return (
      <Video 
        size={size} 
...
export const getFileCategory = (filename, mimeType) => {
  ...
  if (extension.match(/^(js|jsx|ts|tsx|py|java|c|cpp|go|rs|php|html|css|sql|json|xml|yaml|yml|sh|bash)$/) || mime.includes('code')) {
    return 'code';
  }
  if (extension.match(/^(txt|md|markdown|rtf)$/) || mime.startsWith('text/')) {
    return 'text';
  ...
  if (extension.match(/^(mp3|wav|aac|flac|m4a|ogg)$/) || mime.startsWith('audio/')) {
    return 'audio';
  }
  if (extension.match(/^(mp4|webm|avi|mkv|mov)$/) || mime.startsWith('video/')) {
    return 'video';
  }
